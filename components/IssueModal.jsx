...
const EMPTY_ISSUE = { title: "", estimatedHours: "", description: "", labels: [], priority: "Medium", dueDate: "", recurrence: "None" };
    ...
    return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
        <form className="task-modal" onSubmit={save} onMouseDown={event => event.stopPropagation()}>
            ...
            {isEditing ? <>
                ...
                <div className="issue-schedule"><label>Due date<input type="date" value={form.dueDate || ""} onChange={event => setForm({ ...form, dueDate: event.target.value })} /></label><label>Repeat<select value={form.recurrence || "None"} onChange={event => setForm({ ...form, recurrence: event.target.value })}><option>None</option><option>Daily</option><option>Weekly</option><option>Monthly</option></select></label></div>
            ...
            </> : activeTab === "details" ? <div className="issue-details"><p><b>Tags</b>{labels.length ? <span className="detail-tags">{labels.map(label => <span className="board-label" key={label.name} style={{ "--label-color": label.color }}>{label.name}</span>)}</span> : "None"}</p><p><b>Priority</b>{issue.priority || "Medium"}</p><p><b>Due date</b>{issue.dueDate ? new Date(`${issue.dueDate}T00:00:00`).toLocaleDateString() : "Not set"}</p><p><b>Repeat</b>{issue.recurrence || "None"}</p><p><b>Description</b>{issue.description || "No description"}</p><p><b>Estimated time</b>{issue.estimatedHours ? `${issue.estimatedHours}h` : "Not set"}</p><section className="activity-list"><b>Activity</b>{issue.activity?.length ? issue.activity.map(entry => <p key={entry.id}>{entry.text}<time>{new Date(entry.createdAt).toLocaleString()}</time></p>) : <span className="empty-state">No status changes yet.</span>}</section></div> : <section className="issue-comments"><h3>Comments</h3>{issue.comments?.length ? <div className="comment-list">{issue.comments.map(entry => <article className="comment" key={entry.id}><time>{new Date(entry.createdAt).toLocaleString()}</time><p>{entry.text}</p></article>)}</div> : <p className="empty-state">No comments yet.</p>}<div className="comment-form"><textarea value={comment} onChange={event => setComment(event.target.value)} placeholder="Add a progress note" /><button className="secondary-button" type="button" onClick={addComment}><MessageSquarePlus size={16} /> Add comment</button></div></section>}
